...
def compute_outcomes(results: list[dict], openai_key: str) -> list[dict]:
    ...
    for item in results:
        ...
        for model_key, traj in item["trajectory"].items():
            obs = traj["obs"]
            es = obs == food
            ewc = cosine_similarity(food_emb, emb_cache[obs.strip()]) if food_emb and obs.strip() in emb_cache else None
